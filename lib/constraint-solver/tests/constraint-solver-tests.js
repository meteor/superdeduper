...
var testWithResolver = function (resolver, f) {
  ...
  var FAIL = function (deps, regexp) {
    try {
      ...
      if (_.isFunction(regexp)) {
        assert.isTrue(!!regexp(error), error.toString());
      } else {
        regexp.test(error.message, error.toString());
      }
    }
  };
  f(t, FAIL);
...
describe("constraint-solver", function() {
  ...
  it("input serialization", function () {
    ...
    assert.sameMembers(input1.dependencies, ["sparky-forms"]);
    ...
    assert.sameMembers(input1.upgrade, []);
    assert.deepEqual(input1.anticipatedPrereleases, {});
    ...
    assert.deepEqual(input2prime.toJSONable(), {
      dependencies: ["foo"],
