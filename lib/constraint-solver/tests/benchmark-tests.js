...
const PackageVersion = require('../package-version-parser/package-version.js');
const splitArgs = require('./helpers.js').splitArgs;

const gems = require('./gem-test-data.js');
const railsCatalog = getCatalogStub(gems.railsGems);
const sinatraCatalog = getCatalogStub(gems.sinatraGems);

if (runBenchmarks) {
  describe('constraint solver', function() {
    describe('benchmark on gems', function() {
      it('resolves sinatra', function() {
        var r = new PackagesResolver(sinatraCatalog);
      ...
      it('resolves rails', function() {
        var r = new PackagesResolver(railsCatalog);
      ...
      it('resolves rails, gitlabhq', function() {
        this.timeout(5000);
        var r = new PackagesResolver(railsCatalog);
      ...
      it ('resolves rails, gitlabhq, additions to the existing smaller solution', function() {
        this.timeout(5000)
        var r = new PackagesResolver(railsCatalog);
        ...
        _.each(args.dependencies, function (dep) {
          if (previousSolution[dep])
            assert.deepEqual(solution[dep], previousSolution[dep], dep);
        });
      });
    });
  });
...
function getCatalogStub (gems) {
  return {
    ...
    getVersion(packageName, version) {
      return _.find(this.getSortedVersionRecords(packageName), vr => vr.version === version);
    }
  };
}
