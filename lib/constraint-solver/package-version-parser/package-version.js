...
var PV = function (versionString) {
  ...
  // XXX: we want to relax into using full npm semver. However we need to do
  //   more than just removing this check -- ie we need to properly support it everywhere
...
// XXX: in progress expanding this to support full npm-compat ranges
...
var parseSimpleConstraint = function (constraintString) {
  ...
  if (constraintString === '*' || constraintString === 'x' || constraintString === 'latest') {
    return {
      type: 'any-reasonable', versionString: ''
    };
  }

  // XXX: this is just temporary
  if (constraintString.charAt(0) === '~' || constraintString.charAt(0) === '^') {
    type = 'compatible-with';
    versionString = constraintString.substr(1);
  } else if (constraintString.substring(0, 2) === '>=') {
    type = 'compatible-with';
    versionString = constraintString.substr(2);
  } else if (constraintString.charAt(0) === '=') {
    type = "exactly";
    versionString = constraintString.substr(1);
  } else {
    type = "compatible-with";
  ...
  versionString = versionString.replace(/x|\*/g, '0');

  if (!versionString.match(/^\d+\.\d+/)) {
    versionString = versionString + '.0.0';
  } else if (!versionString.match(/^\d+\.\d+\.\d+/)) {
...
PV.VersionConstraint = function (vConstraintString) {
  ...
  if (! vConstraintString) {
    ...
    alternatives = _.map(parts, function (alt) {
      if (! alt) {
...
PV.validatePackageName = function (packageName, options) {
  ...
  // XXX: changed to support npm
  var badChar = packageName.match(/[^a-zA-Z0-9:.\-_@/]/);
  if (badChar) {
  ...
  if (packageName[0] === ":" || _.last(packageName) === ":") {
