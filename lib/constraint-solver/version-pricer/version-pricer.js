...
VersionPricer.MODE_UPDATE = 1;
VersionPricer.MODE_GRAVITY = 2;
VersionPricer.MODE_GRAVITY_WITH_PATCHES = 3;
...
// the versions array.
VersionPricer.prototype.priceVersions = function (versions, mode, options) {
  var self = this;
  ...
  switch (mode) {
  case VersionPricer.MODE_UPDATE:
    gravity = [false, false, false, false];
    break;
  case VersionPricer.MODE_GRAVITY:
    gravity = [true, true, true, true];
    break;
  case VersionPricer.MODE_GRAVITY_WITH_PATCHES:
...
// higherMajor: ["3.0.0"] }`.
VersionPricer.prototype.partitionVersions = function (versions, target) {
  var self = this;
...
VersionPricer.prototype.priceVersionsWithPrevious = function (
  versions, target, takePatches) {

  var self = this;
  var parts = self.partitionVersions(versions, target);

  var result1 = self.priceVersions(parts.older, VersionPricer.MODE_UPDATE,
                                   { versionAfter: target });
