...
const VersionPricer = require('../version-pricer/version-pricer.js');
...
const Solver = function (input, options) {
  var self = this;
  ...
  self.Profile = (self.options.Profile || DummyProfile);
...
Solver.prototype.throwAnyErrors = function () {
  if (this.errors.length) {
    var multiline = _.some(this.errors, function (e) {
      return /\n/.test(e);
    });
    throwConstraintSolverError(this.errors.join(
      multiline ? '\n\n' : '\n'));
  }
};

Solver.prototype.getVersions = function (pkg) {
  var self = this;
...
Solver.prototype.analyze = function () {
  var self = this;
    ...
    _.each(_.groupBy(input.constraints, 'package'), function (cs, p) {
      ...
      _.each(cs, function (constr) {
        versions = _.filter(versions, function (v) {
          return isConstraintSatisfied(p, constr.versionConstraint, v);
        });
      });
      if (! versions.length) {
    ...
    _.each(input.dependencies, function (p) {
      if (! input.isKnownPackage(p)) {
        analysis.unknownRootDeps.push(p);
                 ...
                 ! input.isUpgrading(p)) {
        analysis.previousRootDepVersions.push(new PackageAndVersion(
  ...
  Profile.time("analyze constraints", function () {
    ...
    _.each(input.constraints, function (c) {
      if (c.constraintString) {
        analysis.constraints.push(new Solver.Constraint(
...
//
// options is optional.
Solver.Step = function (name, terms, weights) {
  // check(name, String);
  terms = terms || [];
  // check(terms, [String]);
...
Solver.Step.prototype.addTerm = function (term, weight) {
  if (weight == null) {
...
Solver.prototype.minimize = function (step, options) {
  var self = this;
...
var groupMutuallyExclusiveTerms = function (costTerms, costWeights) {
  ...
  if (typeof costWeights === 'number') {
    ...
    var newWeightedTerms = _.map(_.groupBy(weightedTerms, function (wt) {
      ...
      return [wts[0][0], Logic.or(_.map(wts, 1))];
    });
    return {
      costTerms: _.map(newWeightedTerms, 1),
      costWeights: _.map(newWeightedTerms, 0)
    };
  }
...
Solver.prototype.getStepContributions = function (step) {
  var self = this;
...
Solver.prototype.getVersionCostSteps = function (stepBaseName, packages,
                                                    pricerMode) {
  var self = this;
  var major = new Solver.Step(stepBaseName + '_major');
  var minor = new Solver.Step(stepBaseName + '_minor');
  var patch = new Solver.Step(stepBaseName + '_patch');
  var rest = new Solver.Step(stepBaseName + '_rest');
...
Solver.prototype.getVersionDistanceSteps = function (stepBaseName,
                                                        packageAndVersions,
                                                        takePatches) {
  var self = this;

  var incompat = new Solver.Step(stepBaseName + '_incompat');
  var major = new Solver.Step(stepBaseName + '_major');
  var minor = new Solver.Step(stepBaseName + '_minor');
  var patch = new Solver.Step(stepBaseName + '_patch');
  var rest = new Solver.Step(stepBaseName + '_rest');
...
Solver.prototype.setSolution = function (solution) {
  var self = this;
...
Solver.prototype.getAnswer = function (options) {
  var self = this;
...
Solver.prototype._getAnswer = function (options) {
  var self = this;
  var input = self.input;
  var analysis = self.analysis;
  var cache = input.catalogCache;
  ...
  self.minimize('conflicts', _.map(analysis.constraints, 'conflictVar'),
                { strategy: 'bottom-up' });
  ...
  self.minimize(self.getVersionCostSteps(
    'update', toUpdate, VersionPricer.MODE_UPDATE));
  ...
  var otherPrevious = _.filter(_.map(input.previousSolution, function (v, p) {
    return new PackageAndVersion(p, v);
  }), function (pv) {
    var p = pv.package;
    return analysis.reachablePackages[p] === true &&
  ...
  self.minimize(self.getVersionCostSteps(
    'new_root', newRootDeps, VersionPricer.MODE_UPDATE));
  ...
  self.minimize(self.getVersionCostSteps(
    ...
    VersionPricer.MODE_GRAVITY_WITH_PATCHES));
  ...
  if ((! input.allowIncompatibleUpdate) &&
    ...
    Profile.time("generate errors for incompatible root change", function () {
      ...
      _.each(incompatRootChanges, function (pvStr) {
        var pv = PackageAndVersion.fromString(pvStr);
...
var getOkVersions = function (toPackage, vConstraint, targetVersions) {
  return _.compact(_.map(targetVersions, function (v) {
    if (isConstraintSatisfied(toPackage, vConstraint, v)) {
...
Solver.prototype.listConstraintsOnPackage = function (pkg) {
  var self = this;
    ...
    if (c.toPackage === pkg) {
      ...
      if (c.fromVar) {
        paths = self.getPathsToPackageVersion(
          PackageAndVersion.fromString(c.fromVar));
      } else {
        paths = [['top level']];
      }
...
Solver.prototype.throwConflicts = function () {
  var self = this;
...
Solver.prototype.getPathsToPackageVersion = function (packageAndVersion) {
  // check(packageAndVersion, PackageAndVersion);
  var self = this;
  var input = self.input;
  var cache = input.catalogCache;
  var solution = self.solution;
  ...
  var getPaths = function (pv, _ignorePackageSet) {
    ...
    _.each(allPackages, function (p) {
      if ((! _.has(newIgnorePackageSet, p)) &&
          solution.evaluate(p) &&
          hasDep(p, pkg)) {
        var newPV = new PackageAndVersion(p, versionMap[p]);
...
Solver.Constraint = function (fromVar, toPackage, vConstraint, conflictVar) {
...
const isConstraintSatisfied = function (pkg, vConstraint, version) {
...
const isIsobuildFeaturePackage = function (packageName) {
