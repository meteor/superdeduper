...
CatalogCache.prototype.hasPackageVersion = function (pkg, version) {
  return _.has(this._dependencies, pvkey(pkg, version));
};

CatalogCache.prototype.addPackageVersion = function (p, v, deps) {
  // check(p, String);
  // check(v, String);
...
CatalogCache.prototype.getDependencyMap = function (p, v) {
  var key = pvkey(p, v);
  if (! _.has(this._dependencies, key)) {
...
CatalogCache.prototype.getPackageVersions = function (pkg) {
...
CatalogCache.prototype.hasPackage = function (pkg) {
  return _.has(this._versions, pkg);
};

CatalogCache.prototype.toJSONable = function () {
  var self = this;
  var data = {};
...
CatalogCache.prototype.eachPackageVersion = function (iter) {
  var self = this;
  _.find(self._dependencies, function (value, key) {
    var stop = iter(PackageAndVersion.fromString(key), value);
    return stop;
  });
};
...
CatalogCache.prototype.eachPackage = function (iter) {
  var self = this;
