...
const CatalogLoader = function (fromCatalog, toCatalogCache) {
  var self = this;
...
// XXX: NOTE: converted to filter out constraints we don't understand
//  (github and ranges in both directions). This is obviously an issue
var convertDeps = function (catalogDeps) {
  return _.compact(_.map(catalogDeps, function (dep, pkg) {
    // The dependency is strong if any of its "references"
    // (for different architectures) are strong.
    var isStrong = _.some(dep.references, function (ref) {
      return !ref.weak;
    });

    var constraint = (dep.constraint || null);

    if (constraint && !constraint.match(/^[\^\~]?[0-9x*]+\.[0-9x*]+\.[0-9x*]+$/)) {
      return false;
    }

    try {
      // test out for junk like 1.2.3beta
      new require('semver').Range(constraint);
    } catch (err) {
      return false;
    }

    return new Dependency(new PV.PackageConstraint(pkg, constraint),
...
CatalogLoader.prototype._getSortedVersionRecords = function (pkg) {
...
CatalogLoader.prototype.loadSingleVersion = function (pkg, version) {
  var self = this;
...
CatalogLoader.prototype.loadAllVersions = function (pkg) {
  var self = this;
  var cache = self.catalogCache;
