...
const PackagesResolver = require('../index.js');
...
var doTest = function (inputJSONable, outputJSONable, options) {
  var input;
  if (inputJSONable instanceof Input) {
    input = inputJSONable;
  } else {
    input = Input.fromJSONable(inputJSONable);
  }
  ...
  assert.equal(
    formatSolution(PackagesResolver._resolveWithInput(input, options)),
    formatSolution(outputJSONable));
};

var doFailTest = function (inputJSONable, messageExpect) {
  var input;
  if (inputJSONable instanceof Input) {
    input = inputJSONable;
  } else {
  ...
  assert.throws(function () {
    try {
      PackagesResolver._resolveWithInput(input);
    } catch (e) {
...
describe('constraint solver', function() {
  describe('input', function() {
    it('can upgrade indirect dependency', function() {
      doTest({
        dependencies: ["foo"],
        constraints: [],
    ...
    it("can upgrade direct, don't break", function() {
      doTest({
    ...
    it('previous solution no patch', function() {
      doTest({
        dependencies: ["foo"],
        constraints: [],
        previousSolution: { foo: "1.0.0", bar: "2.0.0" },
    ...
    it("don't break root dep", function() {
      doTest({
        dependencies: ["foo", "bar"],
        constraints: [],
    ...
    it("previous solution no longer needed", function() {
      doTest({
        dependencies: ["foo"],
        constraints: [],
    ...
    it("conflicting top-level constraints", function() {
    ...
    it("previous indirect deps", function() {
      doTest({
        dependencies: ["a"],
        constraints: [],
    ...
    it("fake PackageConstraint", function() {
      ...
      doFailTest(new Input(["foo", "bar"], [fakeConstraint],
                              CatalogCache.fromJSONable({
                                data: {
                                  "foo 1.0.0": [],
                                  "foo 2.0.0": [],
                                  "bar 1.0.0": ["foo@1.0.0"]
    ...
    it("stack overflow bug", function() {
      ...
      doFailTest(require('./stack-overflow-bug-test-data.js'),
                 /No version of follower-livedata satisfies all constraints: @0.9.0/);
    });


    it("bad package name", function() {
      assert.throws(function () {
        new Input(['-x'], [], new CatalogCache());
      }, /may not begin with a hyphen/);

      assert.throws(function () {
        new Input([], [], new CatalogCache(),
    ...
    it("slow solve", function() {
      var input = Input.fromJSONable(require('./slow-test-data.js'));

      assert.equal(
        formatSolution(PackagesResolver._resolveWithInput(input)),
        formatSolution({
          "answer":{
            "autopublish":"1.0.2",
    ...
    it("update indirect deps", function() {
    ...
    it("package is only weak dep", function() {
      doTest({
        dependencies: ["foo"],
        constraints: [],
        previousSolution: {},
